...
use src\Services\DatabaseUsers;
...
class UserRepository
{
    ...
    {
        try {
            $query = 'INSERT INTO users (firstName, lastName, email, password, is_activated, token, profile_picture_path, rgpd_date, created_at, role_id) 
    ...
    {
        try {
            $query = 'UPDATE users SET activationToken = :token WHERE user_id = :user_id';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query = 'UPDATE users SET reset_password_token = :reset_password_token WHERE user_id = :user_id';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query = 'SELECT u.*, r.name AS role_name FROM users u JOIN roles r ON u.role_id = r.role_id WHERE u.email = :email';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query  = 'SELECT user_id, token FROM users WHERE token = :token';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query  = 'SELECT user_id, reset_password_token FROM users WHERE reset_password_token = :resetPasswordToken';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query = 'UPDATE users SET reset_password_request_time = :reset_password_request_time WHERE user_id = :user_id';
            $req = $this->DBuser->prepare($query);
    ...
    {
        try {
            $query = 'UPDATE users SET is_activated = 1, token = NULL WHERE user_id = :user_id';
            $req = $this->DBuser->prepare($query);
            $req->execute(['user_id' => $user_id]);
            return true;
        } catch (Exception $e) {
    ...
    {
        try {
            $query = 'UPDATE users SET first_name = :first_name, last_name = :last_name, email = :email, updated_at = :updated_at WHERE user_id = :user_id';
            $req = $this->DBuser->prepare($query);
            $req->execute([
    ...
    {
        try {
            $query = $this->DBuser->prepare('UPDATE users SET password = :password WHERE user_id = :user_id');
            $query->execute([
    ...
    {
        try {
            $query = $this->DBuser->prepare('UPDATE users SET password = :password , reset_password_token = NULL WHERE user_id = :user_id');
            $query->execute([
    ...
    public function deleteUser($user_id)
    {
        try {
            ...
            $query = $this->DBuser->prepare('DELETE FROM users WHERE user_id = :user_id');
    ...
    {
        $query = $this->DBuser->prepare('UPDATE users SET profile_picture_path = :profile_picture_path WHERE user_id = :user_id');
        $query->execute([
