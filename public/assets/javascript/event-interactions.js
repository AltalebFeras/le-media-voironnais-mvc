const EventInteractions = (function () {
  ...
  function updateUI() {
    ...
    if ($likeIconEl.length) {
      $likeIconEl.html(
        interactionData.userHasLiked
          ? SVG_ICONS.likeActive
          : SVG_ICONS.likeInactive
      );
    }
    ...
    if ($favouriteIconEl.length) {
      $favouriteIconEl.html(
        interactionData.userHasFavourited
          ? SVG_ICONS.favouriteActive
          : SVG_ICONS.favouriteInactive
      );
    }
  ...
  function renderComments() {
    ...
    comments.forEach((comment) => {
      ...
      html += renderComment(
        comment,
        repliesByParent[comment.idEventComment] || []
      );
    });
  ...
  function renderComment(comment, replies = []) {
    ...
    const canDelete =
      isLoggedIn && currentUserUiid && comment.userUiid == currentUserUiid;
    const isOpen = openRepliesSections.has(comment.uiid);

    let html = `
            <div class="comment" data-uiid="${
              comment.uiid
            }" data-author="${escapeHtml(comment.firstName)} ${escapeHtml(
      comment.lastName
    )}">
                <b>${escapeHtml(comment.firstName)} ${escapeHtml(
      comment.lastName
    )}</b>
                ...
                <div>
                    ...
                    ${
                      isLoggedIn
                        ? `
                        <button class="like-comment-btn" data-uiid="${
                          comment.uiid
                        }" style="background:none;border:none;vertical-align:middle;cursor:pointer;">
                            ${SVG_ICONS.commentLike}
                        </button>
                        <button class="reply-comment-btn" data-uiid="${
                          comment.uiid
                        }" data-parent="${
                            comment.uiid
                          }" style="background:none;border:none;vertical-align:middle;cursor:pointer;">
                        ...
                        <button class="report-comment-btn" data-uiid="${
                          comment.uiid
                        }" style="background:none;border:none;vertical-align:middle;cursor:pointer;">
    ...
    if (hasReplies) {
      html += `<div class="replies" id="replies-${
        comment.uiid
      }" style="margin-left:2em;display:${isOpen ? "block" : "none"};">`;
      replies.forEach((reply) => {
        const replyFormattedDate = formatCommentDate(reply.createdAt);
        const canDeleteReply =
          isLoggedIn && currentUserUiid && reply.userUiid == currentUserUiid;

        html += `
                    <div class="comment reply" data-uiid="${
                      reply.uiid
                    }" data-author="${escapeHtml(reply.firstName)} ${escapeHtml(
          reply.lastName
        )}">
                        <b>${escapeHtml(reply.firstName)} ${escapeHtml(
          reply.lastName
        )}</b>
                        ...
                        <div>
                            ...
                            ${
                              isLoggedIn
                                ...
                                <button class="reply-comment-btn" data-uiid="${
                                  reply.uiid
                                }" data-parent="${reply.uiid}" data-root="${
                                    comment.uiid
                                  }" style="background:none;border:none;vertical-align:middle;cursor:pointer;">
                                    ${SVG_ICONS.commentReply}
                                </button>
      ...
      html += `</div>
                <button class="show-replies-btn" data-uiid="${
                  comment.uiid
                }" style="cursor:pointer;">
                    ${
                      isOpen
                        ? "Masquer les réponses"
                        : `Voir toutes les ${replies.length} réponse${
                            replies.length > 1 ? "s" : ""
                          }`
                    }
                </button>`;
            ...
            <form class="reply-form" data-parent="${
              comment.uiid
                ...
                <div style="margin-bottom:0.5em;">
                    <small style="color:#666;">Répondre à <strong>${escapeHtml(
                      comment.firstName
                    )} ${escapeHtml(comment.lastName)}</strong></small>
                </div>
  ...
  function handleReplyToggle(e) {
    ...
    const authorName = $targetComment.length
      ? $targetComment.data("author")
      : "";

    let $form = $rootCommentDiv.find(
      `.reply-form[data-reply-to="${commentUiid}"]`
    );
    ...
    if ($repliesDiv.length && $repliesDiv.css("display") === "none") {
      ...
      const $showBtn = $rootCommentDiv.find(
        `.show-replies-btn[data-uiid="${rootUiid}"]`
      );
      if ($showBtn.length) {
